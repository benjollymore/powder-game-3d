...
1. Lava reads as a flat cream slab (glow ramp saturates at 1500 K) and the ice cave interior blows out white; rendering worker, with the Examples captures as the fixture.
2. Thermal cost at 256: paired on/off deltas of 5 to 7 ms per frame (throttled, Low Power Mode) against a 3 ms budget; thermal worker is ranking reductions (block early-out, skipping remap lines without liquid change, cheaper buoyancy sampling) behind bench flags. Rerun `tools/milestone/thermal_bench.gd` on mains power before deciding.
