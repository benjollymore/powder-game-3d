...
1. **`Elements.TABLE` v2 keys.** Existing keys unchanged. New keys, all with defaults so old rows keep working: `category` (one of `common`, `heat`, `powders`, `liquids`, `gases`, `solids`, `special`), `tip` (one sentence), `heat_capacity` (J/K per full cell, > 0), `conductivity` (W/(m K)), `initial_temp` (K), `fire_temp` (K, gases that are fire), `ignition_temp` (K, flammables; 0 = never), `hot_at` / `hot_to` (K, id), `cold_at` / `cold_to` (K, id), `latent` (K x capacity units for the plateau), `burn_to` (id). Encoded into an 80-byte `Elem` (`ELEM_BYTES = 80`; std430 aligns the trailing uvec4 to 16 bytes, so 64 was not representable): bytes 0..31 exactly as today, `vec4 thermal_a = (heat_capacity, conductivity, initial_temp, fire_temp)` at 32..47, `vec4 thermal_b = (ignition_temp, hot_at, cold_at, latent)` at 48..63, then `uint ids = hot_to | cold_to << 8 | burn_to << 16` in the first component of a uvec4 at 64..79 (the rest zero). Accessor macros live in `shaders/compute/elem.glslinc`; read thermal values through them. One `shaders/compute/elem.glslinc` replaces every copy of `struct Elem`; a unit check compares `Elements.ELEM_BYTES` with a `ELEM_BYTES_SENTINEL` constant in the include. Element ids: existing 0..9 unchanged; tranche A `ICE=10, LAVA=11, STONE=12, METAL=13`; tranche B `WAX=14, MOLTEN_WAX=15, GUNPOWDER=16, GAS=17, ACID=18, CLONE=19, VOID=20`, `GLASS=21` reserved. `PALETTE_SIZE = 32`.
...
Landed 2026-09-15: contract 1 schema (`dfdbaa7`), palette 32 (`94465ec`), `PALETTE_SIZE = 32`. New element rows use MaterialLibrary layers: ICE = 5, GLASS = 6, LAVA and STONE reuse existing layers with their own colours; the elements worker adds rows and colours, the rendering worker adds the layers and glow.
