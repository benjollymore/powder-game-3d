...
Every cell carries `T` (kelvin) and `G` (latent progress, joules) in an `RG32F` texture beside the packed voxel grid. Energy is `E = C·T + G`, where the capacity `C` is the element's `heat_capacity` (J/K per full cell) scaled for liquids by `amount / 200` with a floor of 4 units, so a film can never carry an absurd temperature. Compressed liquid (amount above 200) holds proportionally more heat. `VoxelSim.energy_total` sums the same model on the CPU.
...
- `spread_row`: the pooled cells mix fully (a 2×2 row has no parcel order); remnants merging into a fuller neighbour carry their energy.
...
- Air displaced by liquid (a partial parcel entering an air cell) loses its energy, bounded by air's capacity (0.05 J/K per cell). The falling-water test records the measured drift.
