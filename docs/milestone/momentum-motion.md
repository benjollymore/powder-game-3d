...
Godot 4.6.3 / Metal 4.0 / Apple M5 Pro passes **84 GPU logical checks**, **11 CPU timestep/shader-import checks**, and **14 binary64 tests** (the existing 10 zero-time tests plus 4 finite-motion tests). All eight numerical evaluations pass their original limits. No runtime errors or resource-leak diagnostics occur. [GPU log](evidence-momentum-gpu/gpu-commit-fixed.log), [numerical evaluation](evidence-momentum-gpu/evaluation-commit-fixed.log), [complete metrics](evidence-momentum-gpu/metrics.json), [CPU tests](evidence-momentum-gpu/cpu-reference.log), [admission/shader checks](evidence-momentum-gpu/admission.log).
...
The first version used several commit invocations which read the halt word while invocation 0 could write it during rejection. Tested outcomes agreed, but that unsynchronized access was still invalid as a correctness foundation. Commit now uses one invocation for the full decision and copy. Reset also clears the grid buffer: an invalid new initial state halts P2G and must not expose the previous run's grid.

The corrected implementation reran every GPU and numerical gate: 84 logical checks pass and all numerical metrics remain unchanged. All 33 initial/checkpoint particle hashes, status arrays and grid values equal the original run. Two rejected-advection checkpoints explicitly compare grid hashes with the last accepted authoritative snapshot; a valid-run→invalid-reset test requires all-zero grid data, and subsequent halted submissions must leave that cleared grid unchanged. The grid is therefore either the transfer grid of the last valid authoritative particles, or cleared after invalid initial admission—not an old world's data or a candidate-position grid. [Comparison](evidence-momentum-gpu/commit-fix-comparison.log), [original raw results](evidence-momentum-gpu/original-results.json), [original metrics](evidence-momentum-gpu/original-metrics.json).
...
The gather implementation loops over all particles for every grid node; a single commit invocation scans and copies the entire particle set. It is deterministic and convenient for tiny validation fixtures; it is deliberately unsuitable as a scalable particle solver. No throughput measurement or production performance claim is made. FP32 error is measured only over these 100-step trajectories and this device, not arbitrary long runs or platforms. There is no gravity and therefore no hidden external impulse or torque ledger.
