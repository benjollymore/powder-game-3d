...
A per-element `liquid_opacity[32]` uniform (row key `opacity`, default 1) multiplies `absorb` for the liquid being traversed in the volume pass, and the opaque pass scales `caustic_strength` by `exp(-0.5 (opacity - 1))` for the liquid above, so water (exp(0) = 1 exactly) keeps its bytes while lava at 12 leaves 0.4 percent. A first version divided by the opacity; that left an 8 percent ripple that still showed through the block's thin silhouette edges where the absorbing path is short. Water and every existing liquid keep opacity 1, so their arithmetic multiplies by exactly 1.0 and the output is byte-identical. Lava (12), molten wax (5) and acid (2) become dense enough to hide what they cover and cast no water ripples. This is a presentation contract for translucency, not fluid physics.

## Validation (pending lease)

`tests/milestone/render_liquid_base_gpu.gd`: water and lava blocks on a slab, front and oblique, baseline shaders versus current, each with caustics on and off. Gates: no falling flags in the block; water byte-identical in both caustic states; lava baseline streak count with caustics at least three times the count without (diagnosis); lava candidate insensitive to `caustic_strength` within the metric's own frame-to-frame noise plus one twentieth of the baseline response, both as streak counts and as differing pixels; physical bytes unchanged. Counts are compared only within one variant: an opaque block's lower band has its own bright base rim, so absolute counts differ between the translucent baseline and the opaque candidate and are not comparable (the first gate wrongly did that). Streaks are pixels in the lower half of the block's screen box brighter than that band's median by 0.12.
