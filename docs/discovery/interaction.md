...
- Two-finger trackpad swipe orbits; Shift + two-finger swipe pans; pinch zooms. Option + primary drag orbits, and Option + Shift + primary drag pans. Plain primary drag still paints. Right drag/wheel remain supported. V/Center restores the centered plane view; F restores a centered angled view. See [Mac trackpad controls and validation](mac-trackpad.md).
- X/Y/Z plane buttons select the construction axis. The −/+ buttons move exactly one cell; the numeric field selects any cell directly. Shift-wheel moves one cell per accumulated wheel unit, respecting high-resolution fractional events. The preview and coordinates show the intended brush center. Grid guides are coarse; the numeric target is exact.
