...
void main() {
	...
	barrier();
	bool in_range = i < pc.m.x;
	Particle q = in_range ? pool.p[i] : Particle(vec4(0.0), vec4(0.0), vec4(0.0));
	float dt = pc.t.x;
	// Dead slots claim spawn requests in one batch per workgroup: a shared
	// counter hands out local slots, one global atomic reserves the range
	// (thousands of contended global atomics per frame cost milliseconds).
	bool want = in_range && q.vel_kind.w == 0.0;
	uint local_slot = want ? atomicAdd(s_dead, 1u) : 0u;
	barrier();
	if (lidx == 0u) {
		s_base = (s_dead > 0u) ? atomicAdd(counters.count[CLAIM], s_dead) : 0u;
	}
	barrier();
	bool alive = in_range && !want;
	if (want) {
		uint claim = s_base + local_slot;
		if (claim < min(counters.count[SPAWNS], uint(pc.t.z))) {
			Spawn sp = spawns.list[claim];
			uint h = hash(uint(sp.vel_seed.w) + i, pc.m.y);
			float life = (sp.pos_kind.w == KIND_EMBER) ? 0.9 + 1.4 * unit(h, 0)
				: (sp.pos_kind.w == KIND_DUST) ? 0.6 + 0.8 * unit(h, 0)
				: 0.25 + 0.3 * unit(h, 0);
			q.pos_life = vec4(sp.pos_kind.xyz, life);
			q.vel_kind = vec4(sp.vel_seed.xyz, sp.pos_kind.w);
	...
	if (alive) {
		...
		age += dt;
		life -= dt;
		if (life <= 0.0 || any(lessThan(pos, vec3(0.0))) || any(greaterThanEqual(pos, vec3(G)))) {
			alive = false;
		} else if (textureLod(fields, pos / G, 0.0).g > 0.5) {
			if (kind == KIND_DUST) {
	...
	barrier();
	if (lidx == 0u && s_alive > 0u) {
		atomicAdd(counters.count[ALIVE], s_alive);
	}
}
