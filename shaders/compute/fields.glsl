...
layout(std430, set = 0, binding = 3) readonly buffer Counters { uint count[32]; } counters;
...
layout(rgba8, set = 0, binding = 7) uniform restrict writeonly image3D physical_overflow;

layout(constant_id = 0) const int GRID = 128;

layout(push_constant, std430) uniform PresentationTime {
	uvec4 cap; // physical capacities; w: sprite emission enabled
...
bool overflow(uint layer) {
	return pc.cap.w != 0u && counters.count[21u + layer] > pc.cap[layer];
}
...
uint stage(ivec3 p) {
	...
	if ((flags & FLAG_LEAFY) != 0u && overflow(1u)) { return 1u; }
...
void main() {
	uint index = gl_GlobalInvocationID.x + uint(GRID) * (gl_GlobalInvocationID.y + uint(GRID) * gl_GlobalInvocationID.z);
	bvec3 fallback = bvec3(overflow(0u), overflow(1u), overflow(2u));
	if (index == 0u) {
		imageStore(physical_overflow, ivec3(0), vec4(vec3(fallback), 0.0));
	}
	// Overflow implies capacity < candidates <= GRID^3, so this existing pass
	// covers every allocated record. Clear all basis/origin/custom components,
	// including off-diagonal leaf rotations; no nondeterministic partial layer.
	if (any(fallback)) {
		for (uint j = 0u; j < 16u; j++) {
			if (fallback.x && index < pc.cap.x) { grains.data[index * 16u + j] = 0.0; }
			if (fallback.y && index < pc.cap.y) { leaves.data[index * 16u + j] = 0.0; }
			if (fallback.z && index < pc.cap.z) { droplets.data[index * 16u + j] = 0.0; }
		}
	}
