...
func _run() -> void:
	...
	check(result.bytes == 12288, "two first-touched 8³ tiles transfer 12288 bytes (12 per cell with thermal), independent of grid size")
	...
	check(result.bytes == 12288, "mixed region and brush commands retain one before-image per tile")
	...
	result = await finish(id)
	check(result.bytes == 6144, "clamped edge region captures only one in-bounds tile (12 bytes per cell with thermal)")
	sim.restore_edit_transaction(result)
	...
	if VoxelCodec.GRID >= 256:
		...
		check(result.bytes == 6144, "rejected region transfers no additional undo tiles")
		sim.restore_edit_transaction(result)
