...
func run() -> void:
	...
	check(editor.undo_bytes == 4096 and editor.redo_bytes == 0, "mixed transaction retains two first-touch tiles (4096 bytes)")
	...
	check(totals() and editor.redo_bytes == 4096, "Undo transfers the retained byte accounting to Redo, without doubling it")
	await history(true)
	...
	if VoxelCodec.GRID >= 256:
		...
		await finish()
		check(await read() == prefix and editor.undo_bytes == 2048 and not editor.edit_message.is_empty(),
	...
	print("REDO_MEASUREMENT grid=%d full_bytes=%d tiny_bytes=4096 action_completion_us=%s" % [VoxelCodec.GRID, original.size(), action_times])
