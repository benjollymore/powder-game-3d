extends SceneTree
## Per-liquid opacity: alternating paused reservoir frame intervals for water
## (opacity 1, must be byte-identical) and lava (opacity 12) against the
## pre-change volume shader. No device timestamp or TPS claim.
signal ready_result(value: Variant)
var sim: Node3D
var failures := 0
var OUT := "res://docs/milestone/liquid-base-evidence/cost"

func _initialize() -> void:
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("output_dir="): OUT = arg.trim_prefix("output_dir=")
	root.get_node("TimeController").paused = true
	create_timer(300).timeout.connect(func(): push_error("Liquid base cost timeout"); quit(2))
	call_deferred("_run")
...
func _run() -> void:
	...
	for fixture in ["water-200","lava-200"]:
		var data := WorldBuilder.empty()
		var count := 0
		var id: int = Elements.Id.WATER if fixture=="water-200" else Elements.Id.LAVA
		for z in range(n/4,3*n/4):
			for y in range(y_lo,y_hi):
				for x in range(n/4,3*n/4):
					data[VoxelCodec.index(x,y,z)] = VoxelCodec.encode(id,17,200)
					count += 1
		fixtures[fixture] = {"bytes":data.to_byte_array(),"count":count}
	var shaders := {}
	for variant in ["original","candidate"]:
		var shader := Shader.new()
		shader.code = FileAccess.get_file_as_string("res://tests/milestone/fixtures/voxel_volume_liquid_base_baseline.gdshader" if variant=="original" else "res://shaders/spatial/voxel_volume.gdshader")
	...
	print("LIQUID_BASE_COST failures=%d"%failures)
